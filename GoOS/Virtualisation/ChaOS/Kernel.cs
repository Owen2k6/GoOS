...
{
    public const string ver = "Version 1.1_02";
    ...
    protected override void Run()
    {
        ...
        try
        {
            ...
            var truedir = Directory.GetCurrentDirectory();
            var falsedir = @"0:\";
            ...
            else if (input.StartsWith("username"))
            {
                ...
                else if (input.EndsWith("current"))
                {
                    clog("Current username: " + username + "\n", Yellow);
                }
                else
                {
            ...
            else if (input.Contains("color"))
            {
                if (input.EndsWith("list"))
                {
                    ...
                    Console.Write("black - Pure light mode, will make you blind");
                ...
                else if (input.EndsWith("black"))
                {
                    SetScreenColor(White, Black);
                }
                ...
                else if (!input.Contains("dark") && input.EndsWith("blue"))
                {
                    SetScreenColor(Black, Blue);
                }
                else if (!input.Contains("dark") && input.EndsWith("green"))
                {
                    SetScreenColor(Black, Green);
                }
                else if (!input.Contains("dark") && input.EndsWith("cyan"))
                {
                ...
                else if (!input.Contains("dark") && input.EndsWith("gray"))
                {
                    SetScreenColor(Black, Gray);
                }
                else if (!input.Contains("dark") && input.EndsWith("red"))
                {
                    SetScreenColor(Black, Red);
                }
                else if (!input.Contains("dark") && input.EndsWith("magenta"))
                {
                    SetScreenColor(Black, Magenta);
                }
                else if (!input.Contains("dark") && input.EndsWith("yellow"))
                {
            ...
            {
                var Hour = RTC.Hour.ToString();
                var Minute = RTC.Minute.ToString();
            ...
            else if (input == "sysinfo")
            {
                ...
                log(" CPU: " + CPU.GetCPUBrandString());
                log(" CPU speed: " + CPU.GetCPUCycleSpeed() / 1e6 + "MHz");
                log(" System RAM: " + GCImplementation.GetUsedRAM() + "MB used out of " + CPU.GetAmountOfRAM() +
                    "MB\n");
            }
            ...
            else if (input == "diskinfo")
            {
                ...
                var diskSpace = VFSManager.GetTotalSize(@"0:\");
                var fsType = VFSManager.GetFileSystemType("0:\\");
                clog("Disk info for " + FS.GetFileSystemLabel(rootdir), DarkGreen);
                if (diskSpace < 1e6)
                    log(" Disk space: " + availableSpace / 1e3 + " KB free out of " + diskSpace / 1000 + " KB total");
                else if (diskSpace > 1e6)
                    log(" Disk space: " + availableSpace / 1e6 + " MB free out of " + diskSpace / 1e+6 + " MB total");
                else if (diskSpace > 1e9)
                    log(" Disk space: " + availableSpace / 1e9 + " GB free out of " + diskSpace / 1e+9 + " GB total");
            ...
            else if (input.StartsWith("mkdir"))
            {
                try
                {
                    inputBeforeLower = inputBeforeLower.Split("mkdir ")[1];
                }
                catch
                {
                    clog("No arguments\n", Red);
            ...
            else if (input.StartsWith("mkfile"))
            {
                try
                {
                    inputBeforeLower = inputBeforeLower.Split("mkfile ")[1];
                }
                catch
                {
                    clog("No arguments\n", Red);
            ...
            else if (input.StartsWith("deldir"))
            {
                try
                {
                    inputBeforeLower = inputBeforeLower.Split("deldir ")[1];
                }
                catch
                {
                    clog("No arguments\n", Red);
            ...
            else if (input.StartsWith("delfile"))
            {
                try
                {
                    inputBeforeLower = inputBeforeLower.Split("delfile ")[1];
                }
                catch
                {
                    clog("No arguments\n", Red);
            ...
            else if (input.StartsWith("cd") && disk)
            {
                ...
                else if (input.StartsWith("cd "))
                {
                    try
                    {
                        inputBeforeLower = inputBeforeLower.Split("cd ")[1];
                    }
                    catch
                    {
                        clog("No arguments\n", Red);
