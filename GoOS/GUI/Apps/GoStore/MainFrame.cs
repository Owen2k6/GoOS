...
public class MainFrame : Window
{
    ...
    public static readonly List<string> AllowDLFrom = new()
    {
        "1.5"
    };
    ...
    private readonly string sampleInfoFile = "GoOS Test|Test application for GoOS|1.0|Owen2k6|Utilities|1.5|test|goexe";
    ...
    private string[] Catagories = { "ERROR" }; // Hardcoded fallback
    ...
    public override void HandleRun()
    {
        ...
        var width = _infoTextWidthPx >= 0 ? _infoTextWidthPx : Font_1x.MeasureString(_infoBoardText);
    ...
    private void Render(string category)
    {
        ...
        if (_regionBlocked)
        {
            ...
            var line1 = RegionBlockTitle;
            var line2 = _regionBlockReason;
            var line3 = "If you believe this is in error, contact staff on Discord or email help@owen2k6.com.";
        ...
        for (var i = page * 18; i < Math.Min(_repoFiles.Count, page * 18 + 18); i++)
        {
            ...
            var appCat = GetCatagoryIndex(_repoFiles[i].Category);
            ...
            if (appCat == catagory)
            {
                var appName = _repoFiles[i].Name;
                var goosVer = _repoFiles[i].GoOSVersion.TrimEnd();
                var padCount = 25 - appName.Length - (goosVer.Length + 6);
                if (padCount < 0) padCount = 0;
                var VersionSpaces = new string(' ', padCount);

                var x = 150 + Colum * (207 + 5);
                var y = 45 + Line * (78 + 5);
