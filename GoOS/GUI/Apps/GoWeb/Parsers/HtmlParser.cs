...
{
    private static readonly string[] _singletons =
    {
        "area",
        "base",
        "br",
    ...
    private static string DecodeEntities(string str)
    {
        ...
        for (var i = 0; i < str.Length; i++)
        {
            var c = str[i];
            if (c == '&')
                foreach (var (name, entity) in _namedEntities)
                {
                    if (name.Length > str.Length - i) continue;
    ...
    public static Element Parse(string text)
    {
        ...
        text = text.Replace("\r\n", "\n");
        text = text.Trim();
        if (text.StartsWith("<!")) startingIndex = text.IndexOf('>') + 1;
        ...
        for (var i = startingIndex; i < text.Length; i++)
        {
            ...
            if (c == '<')
            {
                ...
                else
                {
                    ...
                    Push(element);
                    if (IsSingleton(tag) || selfClosed) Pop();
                }
            }
            else
            {
