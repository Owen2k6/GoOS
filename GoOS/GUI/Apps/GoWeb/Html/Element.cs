...
{
    private static readonly string[] _blockLevelTags =
    {
        "address",
        "article",
        "aside",
    ...
    public virtual string TextContent
    {
        get
        {
            ...
            foreach (var child in Children) result += child.TextContent;
            return result;
        }
    }
    ...
    public List<Element> Children { get; init; } = new();
    ...
    public bool IsBlockLevel()
    {
        return _blockLevelTags.Contains(GetTag());
    }
    ...
    public bool Selector(string selector)
    {
        ...
        if (selector[0] == '.')
        {
            ...
            if (classes != null)
                foreach (var @class in classes.Split(null))
                    if (@class == checkClass)
                        return true;
        }
    ...
    {
        if (element == this || Children.Contains(element)) return;
    ...
    {
        if (Parent != null) return Parent.GetUnderline();
        return false;
    }

    public virtual bool IsPreformatted()
    {
        if (Parent != null) return Parent.IsPreformatted();
        return false;
    }
    ...
    {
        if (!IsVisible()) return;
        foreach (var child in Children) child.Render(ctx);
    }
