...
namespace GoOS.GUI
{
    internal sealed class WindowManager : Process
    {
        ...
        private static Window _focusedWindowOverride = null;

        internal static Window FocusedWindow
        {
            get
            {
                if (_focusedWindowOverride != null)
                    return _focusedWindowOverride;

                return Windows.Count > 0 ? Windows[^1] : SpecialWindows.Count > 0 ? SpecialWindows[^1] : null;
            }
            set
            {
                if (SpecialWindows.Contains(value))
                {
                    _focusedWindowOverride = value;
                    return;
                }

                _focusedWindowOverride = null;
        ...
        {
            if (window.Title == "menubar" || window.Title == "taskbar" || window.Title == "Desktop")
        ...
        internal static void Render()
        {
            ...
            foreach (Window w in SpecialWindows)
            {
                if (w == null)
                {
                    RemoveWindow(w);
            ...
            for (int i = 0; i < Kernel.ProcessScheduler.Processes.Count; i++)
                Screen.DrawString(10, 114 + (i * 16),
                    Kernel.ProcessScheduler.Processes[i].PID + " (" + Kernel.ProcessScheduler.Processes[i].Name + ")",
            ...
            foreach (Window w in Windows)
            {
                ...
                Screen.DrawImage(w.X, w.Y, w._contents, false);
