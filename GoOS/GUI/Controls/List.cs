...
{
    public Color BackgroundColor = new(143, 143, 143);
    ...
    public override void Render()
    {
        if (Selected < 0)
            Selected = 0;
        else if (Selected > Items.Length) Selected = Items.Length - 1;
        ...
        for (var i = 0; i < Items.Length; i++)
            ...
            {
                Contents.DrawString(2, 21 + i * 20, Items[i], Resources.Font_1x, ForegroundColor);
            }
    ...
    {
        if (IsMouseOver) Selected = ((int)MouseManager.Y - Parent.Y - Window.TITLE_BAR_HEIGHT - Y) / 20 - 1;
