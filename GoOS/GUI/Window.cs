...
namespace GoOS.GUI
{
    public class Window : Process
    {
        ...
        {
            get => MouseManager.X > X && MouseManager.X < X + Width && MouseManager.Y > Y &&
        ...
        internal bool IsMouseOver() // Empty comments!
        {
            ...
            if (Focused && isMouseOverArea) return true;
        ...
        public Window(int X, int Y, ushort Width, ushort Height, string Title, bool Borderless = false) : base(Title)
        {
            ...
            this.X = X;
            this.Y = Y;
            this.Width = (ushort)(Width + (Borderless ? 0 : 12));
            this.Height = (ushort)(Height + (Borderless ? 0 : 29));
            ...
            if (!Borderless)
            {
                closeButton = new Button(this, 4, 4, 13, 13, "", true, Resources.closeButton,
                    Resources.closeButtonPressed, true);
                closeButton.Clicked = () => Dispose();

                maximiseButton = new Button(this, this.Width - 33, 4, 13, 13, "", true, Resources.maximize,
                    Resources.maximizePressed, true);
                ...
                minimiseButton = new Button(this, this.Width - 17, 4, 13, 13, "", true, Resources.minimise,
                    Resources.minimisePressed, true);
        ...
        internal virtual void Render()
        {
            ...
            _contents.DrawImage(maximiseButton.X, maximiseButton.Y, maximiseButton.Contents,
                maximiseButton.RenderWithAlpha);
            minimiseButton.Render();
            _contents.DrawImage(minimiseButton.X, minimiseButton.Y, minimiseButton.Contents,
                minimiseButton.RenderWithAlpha);
            ...
            for (int i = 0; i < 6; i++)
                _contents[(Width / 2) + (Resources.Charcoal.MeasureString(Title + " ") / 2),
                    4 + i * 2] = Color.White;
            for (int i = 0; i < 6; i++)
                _contents[(Width / 2) - (Resources.Charcoal.MeasureString(Title + " ") / 2) - 1,
        ...
        internal void RenderControls()
        {
            foreach (Control c in Controls)
            {
                if (c == null) Controls.Remove(c);
                    ...
                    Contents.DrawImage(c.X /* + (c.NoOffset ? 0 : Borderless ? 2 : 6*/,
                        c.Y /* + (c.NoOffset ? 0 : Borderless ? 2 : 22)*/, c.Contents, c.RenderWithAlpha);
            }
        }
        ...
        internal override void HandleRun()
        {
            ...
            {
                // Handle dragging
                if (!Borderless && IsMouseOver() && IsMouseOverTitlebar && !closeButton.IsMouseOver &&
                    !maximiseButton.IsMouseOver && !minimiseButton.IsMouseOver &&
                ...
                if (MouseManager.MouseState == MouseState.Right && IsMouseOver() && !_ctPressed)
                    _ctPressed = true;

                if (MouseManager.MouseState == MouseState.None && IsMouseOver() && _ctPressed)
                    ShowContextMenu();
            ...
            else if (MouseManager.LastMouseState != MouseManager.MouseState && IsMouseOver())
