...
public static partial class Interpreter
{
    ...
    private static bool Interpreting;

    public static void Run(string file)
    {
        if (!File.Exists(file)) HandleError("9xCode", "Script not found!");
    ...
    private static void Interpret(string[] code)
    {
        ...
        var Strings = new Dictionary<string, string> { { "Version", Version } };
        ...
        for (var i = 0; i < code.Length; i++)
            if (Interpreting)
                try
                {
                    ...
                    if (KeyboardManager.TryReadKey(out var k) && k.Modifiers == ConsoleModifiers.Control &&
                        k.Key == ConsoleKeyEx.C) return;

                    var line = code[i].Trim().Replace("\\n", "\n");

                    #region Globals

                    if (ConsoleLib)
                    {
                        if (Colors.ContainsKey("ForeColor")) Colors.Remove("ForeColor");
                        if (Colors.ContainsKey("BackColor")) Colors.Remove("BackColor");
                        if (Integers.ContainsKey("CursorX")) Integers.Remove("CursorX");
                        if (Integers.ContainsKey("CursorY")) Integers.Remove("CursorY");
                        if (Booleans.ContainsKey("Cursor")) Booleans.Remove("Cursor");
                    ...
                    if (TimeLib)
                    {
                        if (Integers.ContainsKey("Milliseconds")) Integers.Remove("Milliseconds");
                        if (Integers.ContainsKey("Seconds")) Integers.Remove("Seconds");
                        if (Integers.ContainsKey("Minutes")) Integers.Remove("Minutes");
                        if (Integers.ContainsKey("Hours")) Integers.Remove("Hours");
                    ...
                    else if (line.StartsWith("Bool"))
                    {
                        if (line.Contains('='))
                        {
                            var key = line.Substring(5, line.IndexOf(" =") - 5);
                            var sub = line.Substring(line.IndexOf("= ") + 2).ToLower();
                    ...
                    else if (line.StartsWith("Color"))
                    {
                        if (line.Contains('='))
                        {
                            var key = line.Substring(6, line.IndexOf(" =") - 6);
                            var sub = line.Substring(line.IndexOf("= ") + 2);
                            ...
                            {
                                if (StringToConsoleColor.TryGetValue(sub, out var colval1))
                            ...
                            {
                                if (StringToConsoleColor.TryGetValue(sub, out var colval2))
                            ...
                            if (Colors.ContainsKey(key)) Colors.Remove(key);

                            if (StringToConsoleColor.TryGetValue(sub, out var colval3))
                    ...
                    else if (line.StartsWith("Int"))
                    {
                        if (line.Contains('='))
                        {
                            var key = line.Substring(4, line.IndexOf(" =") - 4);
                            var sub = line.Substring(line.IndexOf("= ") + 2);
                        ...
                        else if (line.Contains("++"))
                        {
                            ...
                            if (!Integers.TryGetValue(key, out var intval))
                            {
                    ...
                    else if (line.StartsWith("String"))
                    {
                        if (line.Contains('='))
                        {
                            var key = line.Substring(7, line.IndexOf(" =") - 7);
                            ...
                            {
                                var arg = line.Split('>')[2].Trim().Replace("\"", "");

                                if (Strings.TryGetValue(arg.Trim(), out var strval)) arg = strval;
                            ...
                            {
                                Strings.Add(key, Console.ReadKey().KeyChar.ToString());
                            }
                            else
                            {
                                Strings.Add(key,
                    ...
                    else if (_9xGLLib && line.StartsWith("Window"))
                    {
                        if (line.Contains('='))
                        {
                            ...
                            if (Windows.ContainsKey(key)) Windows.Remove(key);

                            if (_9xGLLib && line.Substring(line.IndexOf("= ") + 2).StartsWith("GetWindow"))
                            {
                                var args2 = line.Split('>')[2].Trim().Split(',');
                                ...
                                else if (Strings.TryGetValue(args2[0].Trim(), out var strval))
                                    wnd.Title = strval;

                                if (Integers.TryGetValue(args2[1].Trim(), out var widthval))
                                    width = Convert.ToUInt16(widthval);
                                else
                                    width = Convert.ToUInt16(args2[1].Trim());

                                if (Integers.TryGetValue(args2[2].Trim(), out var heightval))
                    ...
                    else if (line.StartsWith("if"))
                    {
                        ...
                        if (line.Contains('='))
                        {
                            ...
                            if (Integers.TryGetValue(comparation[0].Trim(), out var intval))
                            {
                                comparation[1] = comparation[1].Substring(1, comparation[1].Length - 2);
                                if (comparation[1] != intval.ToString()) i = endif + 1;
                            }
                            else if (Strings.TryGetValue(comparation[0].Trim(), out var strval))
                            {
                                comparation[1] = comparation[1].Substring(comparation[1].IndexOf('"') + 1,
                                    comparation[1].LastIndexOf('"') - 2);
                                if (comparation[1] != strval) i = endif + 1;
                            }
                            else if (Booleans.TryGetValue(comparation[0].Trim(), out var bolval))
                            {
                                comparation[1] = comparation[1].Substring(1, comparation[1].Length - 2);
                                if (comparation[1] != bolval.ToString().ToLower()) i = endif + 1;
                            }
                        }
                        else if (line.Contains('!'))
                        {
                    ...
                    else if (SysLib && line.StartsWith("Goto") && line.Contains(">>"))
                    {
                        ...
                        if (Integers.TryGetValue(arg, out var value)) arg = value.ToString();
                    ...
                    else if ((ConsoleLib && line.StartsWith("Write") && line.Contains(">>")) ||
                             (ConsoleLib && line.StartsWith("Print") && line.Contains(">>")))
                    {
                        var args = line.Split('>')[2].Trim().Split(',');
                        ...
                        foreach (var arg in args)
                        {
                            ...
                            else if (Booleans.TryGetValue(sub, out var bolval))
                                Console.Write(bolval);
                            else if (Integers.TryGetValue(sub, out var intval))
                                Console.Write(intval);
                            else if (Colors.TryGetValue(sub, out var colval))
                                Console.Write(colval);
                            else if (Strings.TryGetValue(sub, out var strval))
                                Console.Write(strval);
                            else
                                Console.Write(sub);
                        }

                        if (line.StartsWith("Print")) Console.Write('\n');
                    }
                    ...
                    {
                        var sub = line.Split('>')[2].Trim();
                        sub = sub.Substring(1, sub.Length - 2);

                        if (Strings.TryGetValue(sub, out var strval)) sub = strval;

                        using (var stream = new FileStream(sub, FileMode.Create))
                        {
                            stream.Close();
                        }
                    }
                    ...
                    else if (IOLib && line.StartsWith("WrFile") && line.Contains(">>"))
                    {
                        ...
                        if (Strings.TryGetValue(args[0].Replace("\"", ""), out var strval1)) args[0] = strval1;

                        if (Strings.TryGetValue(args[1].Remove(0, 1), out var strval2)) args[1] = strval2;

                        using (var stream = new FileStream(args[0].Replace("\"", ""), FileMode.Open))
                        {
                            byte[] bytes;
                            if (args[1].Contains('"'))
                            {
                    ...
                    else if (_9xGLLib && line.StartsWith("DrawLine") && line.Contains(">>"))
                    {
                        ...
                        if (!Windows.TryGetValue(args[0].Trim(), out var wndval))
                        {
                            HandleError("Error", $"Unknown variable at line {i + 1}!");
                            break;
                        }

                        if (!Integers.TryGetValue(args[1].Trim(), out var xval)) xval = Convert.ToInt32(args[1].Trim());

                        if (!Integers.TryGetValue(args[2].Trim(), out var yval)) yval = Convert.ToInt32(args[2].Trim());

                        if (!Integers.TryGetValue(args[3].Trim(), out var xval2))
                            xval2 = Convert.ToInt32(args[3].Trim());

                        if (!Integers.TryGetValue(args[4].Trim(), out var yval2))
                            yval2 = Convert.ToInt32(args[4].Trim());

                        if (!Colors.TryGetValue(args[5].Trim(), out var colval))
                        {
                            if (StringToConsoleColor.TryGetValue(args[5].Trim(), out var colval2))
                                colval = colval2;
                            else
                                HandleError("Syntax Error", $"Unknown Color at line {i + 1}!");
                        }
                    ...
                    else if (_9xGLLib && line.StartsWith("DrawString") && line.Contains(">>"))
                    {
                        ...
                        if (!Strings.TryGetValue(args[1].Trim(), out var strval))
                        {
                        ...
                        if (!Integers.TryGetValue(args[2].Trim(), out var xval)) xval = Convert.ToInt32(args[2].Trim());

                        if (!Integers.TryGetValue(args[3].Trim(), out var yval)) yval = Convert.ToInt32(args[3].Trim());

                        if (!Colors.TryGetValue(args[4].Trim(), out var colval))
                        {
                            if (StringToConsoleColor.TryGetValue(args[4].Trim(), out var colval2))
                                colval = colval2;
                            else
                                HandleError("Syntax Error", $"Unknown Color at line {i + 1}!");
                        }

                        wndval.Contents.DrawFilledRectangle(xval, xval, Convert.ToUInt16(strval.Length * 8), 16, 0,
                            Color.LightGray);
                    ...
                    else if (GoOSLib && line.StartsWith("RegProg") && line.Contains(">>"))
                    {
                        ...
                        if (!Strings.TryGetValue(args[0].Trim(), out var strval))
                        {
        ...
        while (WindowManager.ContainsAWindow(new List<Window>(Windows.Values))) WindowManager.Update();
    }
