...
internal readonly struct Tok
{
    ...
    public Tok(TokKind k, string t, int p)
    {
        Kind = k;
        Text = t;
        Pos = p;
...
internal sealed class Lexer
{
    ...
    public List<Tok> Tokenise()
    {
        ...
        var safety = Math.Max(256, _s.Length * 4); // hard ceiling
        ...
        while (safety-- > 0)
        {
            ...
            if (c == '{')
            {
                t.Add(new Tok(TokKind.LBrace, "{", _i++));
            }
            else if (c == '}')
            {
                t.Add(new Tok(TokKind.RBrace, "}", _i++));
            }
            else if (c == ':')
            {
                t.Add(new Tok(TokKind.Colon, ":", _i++));
            }
            else if (c == ';')
            {
                t.Add(new Tok(TokKind.Semi, ";", _i++));
            }
            else if (c == ',')
            {
                t.Add(new Tok(TokKind.Comma, ",", _i++));
            }
            else if (c == '=' && Peek() == '>')
            {
                t.Add(new Tok(TokKind.Arrow, "=>", _i));
                _i += 2;
            }
            else if (c == '"' || c == '\'')
            {
    ...
    private char Peek()
    {
        return _i + 1 < _s.Length ? _s[_i + 1] : '\0';
    }
}
...
internal sealed class Parser
{
    ...
    private Tok Peek()
    {
        return _i < _t.Count ? _t[_i] : new Tok(TokKind.Eof, "", _i);
    }
    ...
    private Tok Expect(TokKind k)
    {
        var t = Next();
        return t.Kind == k ? t : new Tok(k, "", t.Pos);
    }
...
internal static class Runtime
{
    public static Window BuildWindow(WindowNode n)
    {
        try
        {
            var width = ReadInt(n.Props, "width", 320);
            var height = ReadInt(n.Props, "height", 240);
    ...
    private static void BuildLabel(ScriptWindow w, LabelNode n)
    {
        try
        {
            var x = ReadInt(n.Props, "x", 10);
            var y = ReadInt(n.Props, "y", 10);
            var text = ReadString(n.Props, "text", n.Name);
    ...
    private static void BuildInput(Window w, InputNode n)
    {
        try
        {
            var x = ReadInt(n.Props, "x", 10);
            var y = ReadInt(n.Props, "y", 10);
            var width = ReadInt(n.Props, "width", 120);
            var height = ReadInt(n.Props, "height", 20);
            var placeholder = ReadString(n.Props, "placeholder", "");
            var text = ReadString(n.Props, "text", "");
    ...
    private static void BuildButton(Window w, ButtonNode n)
    {
        try
        {
            var x = ReadInt(n.Props, "x", 10);
            var y = ReadInt(n.Props, "y", 10);
            var width = ReadInt(n.Props, "width", 80);
            var height = ReadInt(n.Props, "height", 24);
            var text = ReadString(n.Props, "text", n.Name);
...
{
    private readonly List<LabelSpec> _labels = new(16);
    ...
    public void AddLabel(LabelSpec spec)
    {
        _labels.Add(spec);
    }
        ...
        if (!_ranDeferred && DeferredCommands != null && DeferredCommands.Count > 0)
        {
            ...
            try
            {
                foreach (var c in DeferredCommands)
                    if (c is MessageCmd m)
                    {
                        Dialogue.Show(Title, m.Text);
                    }
