...
namespace GoOS.Giff
{
    public static class Giff
    {
        ...
        private static readonly string[] Authors = new string[] { "Owen2k6", "GoOS Team" };
    ...
    internal sealed class Lexer
    {
        ...
        public List<Tok> Tokenise()
        {
            ...
            while (safety-- > 0)
            {
                ...
                if (c == '{') t.Add(MakeAdv(TokKind.LBrace, "{", 1));
                else if (c == '}') t.Add(MakeAdv(TokKind.RBrace, "}", 1));
                else if (c == '=') t.Add(MakeAdv(TokKind.Equals, "=", 1));
                else if (c == ':') t.Add(MakeAdv(TokKind.Colon, ":", 1));
                else if (c == ';') t.Add(MakeAdv(TokKind.Semi, ";", 1));
                else if (c == ',') t.Add(MakeAdv(TokKind.Comma, ",", 1));
                else if (c == '(') t.Add(MakeAdv(TokKind.LParen, "(", 1));
                else if (c == ')') t.Add(MakeAdv(TokKind.RParen, ")", 1));
                else if (c == '+') t.Add(MakeAdv(TokKind.Plus, "+", 1));
                else if (c == '-')
                {
                    ...
                    else t.Add(MakeAdv(TokKind.Minus, "-", 1));
                }
                else if (c == '*') t.Add(MakeAdv(TokKind.Multiply, "*", 1));
                else if (c == '%') t.Add(MakeAdv(TokKind.Modulo, "%", 1));
                else if (c == '$') t.Add(MakeAdv(TokKind.Dollar, "$", 1));
                else if (c == '/')
                {
                    ...
                    else t.Add(MakeAdv(TokKind.Divide, "/", 1));
                }
        ...
        private Tok Make(TokKind k, string text) => new Tok(k, text, _i, _line, _col);
    ...
    {
        public readonly List<WindowNode> Windows = new List<WindowNode>(4);
    }
    ...
    internal sealed class WindowNode
    {
        ...
        public readonly List<ListBoxNode> ListBoxes = new List<ListBoxNode>(4);
    ...
    internal sealed class InputNode
    {
        ...
        public readonly List<CommandNode> OnChange = new List<CommandNode>(4);
    }
    ...
    internal sealed class ButtonNode
    {
        ...
        public readonly List<CommandNode> OnClick = new List<CommandNode>(4);
    }
    ...
    internal sealed class ListBoxNode
    {
        ...
        public readonly List<CommandNode> OnSelect = new List<CommandNode>(4);
    }
    ...
    internal sealed class IfCmd : CommandNode
    {
        ...
        public List<CommandNode> TrueBlock = new List<CommandNode>();
        public List<CommandNode> FalseBlock = new List<CommandNode>();
    ...
    internal sealed class Parser
    {
        ...
        private void ParseBlockInto(List<CommandNode> list)
        {
            ...
            while (!AtEnd() && safety-- > 0)
            {
                ...
                if (TryMatchIdent("if"))
                {
                    ...
                    int trueBlockSafety = Math.Max(16, _t.Count);
                    ...
                    if (TryMatchIdent("else"))
                    {
                        ...
                        int falseBlockSafety = Math.Max(16, _t.Count);
        ...
        private void ParsePropertyInto(Dictionary<string, string> dict)
        {
            ...
            string key = keyTok.Text.ToLowerInvariant();
    ...
    internal static class Runtime
    {
        ...
        public static Window BuildWindow(WindowNode n)
        {
            try
            {
                ...
                // Images
                for (int i = 0; i < n.Images.Count; i++) BuildImage(win, n.Images[i]);

                // ListBoxes (drawn)
                for (int i = 0; i < n.ListBoxes.Count; i++) BuildListBox(win, n.ListBoxes[i]);
        ...
        private static void BuildButton(Window win, ButtonNode n)
        {
            ...
            int w = ParseInt(n.Props, "width", 80);
            int h = ParseInt(n.Props, "height", 24);
        ...
        private static void BuildListBox(Window win, ListBoxNode n)
        {
            int x = ParseInt(n.Props, "x", 0);
            int y = ParseInt(n.Props, "y", 0);
            int w = ParseInt(n.Props, "width", 150);
            int h = ParseInt(n.Props, "height", 100);
            ...
            for (int i = 0; i < n.Items.Count; i++)
            {
                ...
                win.Contents.DrawString(x + 4, itemY, n.Items[i], Charcoal, Color.Black, false);
                itemY += 16;
            }

            for (int i = 0; i < n.Items.Count; i++) _variables["listbox_" + n.Name + "_item_" + i] = n.Items[i];
        ...
        private static void BuildPanel(Window win, PanelNode p)
        {
            ...
            for (int i = 0; i < p.ListBoxes.Count; i++)
            {
                var n = p.ListBoxes[i];
                int x = ParseInt(n.Props, "x", 0) + px;
                int y = ParseInt(n.Props, "y", 0) + py;
                int w = ParseInt(n.Props, "width", 150);
                int h = ParseInt(n.Props, "height", 100);
                ...
                {
                    if (itemY >= y + h - 12) break;
                    win.Contents.DrawString(x + 4, itemY, n.Items[k], Charcoal, Color.Black, false);
                    itemY += 16;
                }
        ...
        public static void Execute(List<CommandNode> cmds, Window ctx)
        {
            for (int i = 0; i < cmds.Count; i++)
            {
                ...
                {
                    bool ok = EvaluateCondition(ifc.Condition);
                ...
                else if (cmd is FileReadCmd fr)
                {
                    TryIO(() =>
                    {
                        string path = EvaluateVariables(fr.Path) ?? "";
                ...
                else if (cmd is FileDeleteCmd fd)
                {
                    TryIO(() =>
                    {
                        string p = EvaluateVariables(fd.Path) ?? "";
                ...
                else if (cmd is FileCopyCmd fc)
                {
                    TryIO(() =>
                    {
                        string s = EvaluateVariables(fc.Source) ?? "";
                        string d = EvaluateVariables(fc.Destination) ?? "";
                        File.Copy(s, d, true);
                    }, "Could not copy file");
                }
                else if (cmd is FileExistsCmd fex)
                {
                    string p = EvaluateVariables(fex.Path) ?? "";
                    bool exists = false;
                ...
                else if (cmd is DirDeleteCmd dd)
                {
                    TryIO(() =>
                    {
                        string p = EvaluateVariables(dd.Path) ?? "";
                        if (Directory.Exists(p)) Directory.Delete(p, true);
                    }, "Could not delete directory");
                }
                else if (cmd is DirExistsCmd dex)
                {
                    string p = EvaluateVariables(dex.Path) ?? "";
                    bool exists = false;
                ...
                else if (cmd is VarsSaveCmd vs)
                {
                    TryIO(() =>
                    {
                        string path = EvaluateVariables(vs.Path) ?? "";
                        using (var sw = new StreamWriter(path, false, Encoding.UTF8))
                        {
                            foreach (var kv in _variables)
                            {
                                string v = Convert.ToBase64String(Encoding.UTF8.GetBytes(kv.Value ?? ""));
                ...
                else if (cmd is VarsLoadCmd vl)
                {
                    TryIO(() =>
                    {
                        string path = EvaluateVariables(vl.Path) ?? "";
                ...
                {
                    string url = EvaluateVariables(ngf.Url) ?? "";
                    string dst = EvaluateVariables(ngf.Dest) ?? "";
                    string status;
                    byte[] body;
                ...
                {
                    string url = EvaluateVariables(ngt.Url) ?? "";
                    string status;
                    byte[] body;
                    if (TryHttpGet(url, out body, out status))
                    {
                        string text = SafeDecodeHttpBody(body);
        ...
        private static string EvaluateVariables(string text)
        {
            ...
            while (pos < text.Length)
            {
                ...
                if (i > d + 1)
                {
                    string name = text.Substring(d + 1, i - (d + 1)).ToLowerInvariant();
                    string value;
        ...
        {
            string condition = EvaluateVariables(cond) ?? "";

            // equality
            int eq = condition.IndexOf("==", StringComparison.Ordinal);
            if (eq >= 0)
            {
                string l = condition.Substring(0, eq).Trim();
                string r = condition.Substring(eq + 2).Trim();
                return string.Equals(l, r, StringComparison.OrdinalIgnoreCase);
            }

            // inequality
            int ne = condition.IndexOf("!=", StringComparison.Ordinal);
            if (ne >= 0)
            {
                string l = condition.Substring(0, ne).Trim();
                string r = condition.Substring(ne + 2).Trim();
                return !string.Equals(l, r, StringComparison.OrdinalIgnoreCase);
            }
            ...
            try
            {
                using (var client = new TcpClient())
                {
                    ...
                    using (var s = client.GetStream())
                    {
                        ...
                        var all = ms.ToArray();
                        int sep = IndexOf(all, Encoding.ASCII.GetBytes("\r\n\r\n"));
                        if (sep >= 0)
                        {
        ...
        private static bool TryParseHttpUrl(string url, out string host, out int port, out string path)
        {
            ...
            try
            {
                ...
                string hostPort = (slash >= 0) ? url.Substring(i, slash - i) : url.Substring(i);
        ...
        private static string ReadStatusLine(byte[] all)
        {
            try
            {
                int end = IndexOf(all, Encoding.ASCII.GetBytes("\r\n"));
