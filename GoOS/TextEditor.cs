...
public class TextEditor
{
    ...
    private readonly (string, string)[] SHORTCUTS =
    {
        ("Ctrl+X", "Quit"),
        ("Ctrl+S", "Save"),
        ("Ctrl+I", "Info"),
    ...
    private void Render()
    {
        ...
        if (updatedLinesStart != null)
        {
            ...
            for (var i = (int)updatedLinesStart; i <= updatedLinesEnd; i++)
            {
                var y = i - scrollY + TITLEBAR_HEIGHT;
                ...
                {
                    var line = lines[i].Substring(scrollX, Math.Min(Console.WindowWidth, lines[i].Length - scrollX));
    ...
    private void JumpToPreviousWord()
    {
        ...
        {
            var res = lines[currentLine].Substring(0, linePos - 1).LastIndexOf(' ');
    ...
    {
        var res = lines[currentLine].IndexOf(' ', linePos + 1);
        if (res == -1)
        {
    ...
    private void ShowNotification(string text)
    {
        ...
        Console.SetCursorPosition((Console.WindowWidth - text.Length) / 2,
            Console.WindowHeight - SHORTCUT_BAR_HEIGHT - 1);
        Console.Write($" {text} ");
    }
    ...
    private void RenderPrompt(string question, (string, string)[] shortcuts)
    {
        ...
        var y = Console.WindowHeight - SHORTCUT_BAR_HEIGHT - 1;
        Console.BackgroundColor = ConsoleColor.White;
        Console.ForegroundColor = ConsoleColor.Black;
    ...
    private void Save(bool showFeedback)
    {
        if (path == null)
        {
            RenderPrompt("Path to save to: ", new[] { ("Enter", "Save"), ("Esc", "Cancel") });

            var result = ConsoleUtil.ReadLineEx(new[] { ConsoleKeyEx.Escape });
            path = result.Input;

            if (!path.Contains(@":\")) path = Paths.JoinPaths(@"0:\", path);
    ...
    private void Quit()
    {
        ...
        if (modified)
        {
            RenderPrompt("Save your changes?", new[] { ("Y", "Yes"), ("N", "No"), ("Esc", "Cancel") });
    ...
    private void CutLine()
    {
        if (lines[currentLine] != string.Empty)
        {
            ...
            else
            {
                ...
                if (currentLine >= lines.Count) currentLine--;

                if (linePos >= lines[currentLine].Length) linePos = lines[currentLine].Length - 1;
    ...
    private bool HandleInput()
    {
        ...
        switch (key.Key)
        {
            ...
            default:
                ...
                if (key.KeyChar >= 32 && key.KeyChar <= 126) Insert(key.KeyChar.ToString());
                break;
        }
    ...
    private void RenderUI()
    {
        ...
        var text = "  GoOS Notepad 2.0";
        Console.WriteLine(text + new string(' ', Console.WindowWidth - text.Length));

        var displayName = path == null ? "New File" : Path.GetFileName(path);
